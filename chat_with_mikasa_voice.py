...
def generate_and_play_voice(text, voice_queue):
    """Generate and play TTS audio in background"""
    global _TTS_PIPELINE, _AUDIO_PROCESS

    if not _TTS_PIPELINE or not _VOICE_ENABLED:
        return
    ...
    try:
        ...
        generator = _TTS_PIPELINE(text, voice='af_heart', speed=1.0)
        ...
        if audio_chunks:
            ...
            with tempfile.NamedTemporaryFile(suffix='.wav', delete=False) as tmp_file:
                sf.write(tmp_file.name, complete_audio, 24000)
                voice_queue.put(('audio', tmp_file.name))

                # Play with afplay and wait for it to complete
                _AUDIO_PROCESS = subprocess.Popen(['afplay', tmp_file.name])
                _AUDIO_PROCESS.wait()  # Wait for audio to finish playing

    except Exception as e:
        print(f"TTS error: {e}")
        voice_queue.put(('error', str(e)))

def bot_response(history, voice_enabled):
    """Generate bot response with image switching and voice"""
    ...
    # Stop any existing audio when new message starts
    if _AUDIO_PROCESS and _AUDIO_PROCESS.poll() is None:
        _AUDIO_PROCESS.terminate()
        _AUDIO_PROCESS.wait()
    ...
    # Trigger voice after text is complete
    if voice_enabled and final_message and _TTS_PIPELINE:
        voice_thread = Thread(
            target=generate_and_play_voice,
            args=(final_message["content"], voice_queue)
        )
        voice_thread.start()

        # Wait for voice generation and playback to complete
        voice_thread.join()
    ...
    with gr.Blocks(title=title) as demo:
        ...
        with gr.Row():
            ...
            with gr.Column(scale=1):
                ...
                if _TTS_PIPELINE:
                    gr.Markdown("🎵 **Voice:** af_heart (Kokoro TTS)")
                else:
                    gr.Markdown("🔇 **Voice:** Not available")
        ...
        def clear_with_audio_stop():
            global _AUDIO_PROCESS
            if _AUDIO_PROCESS and _AUDIO_PROCESS.poll() is None:
                _AUDIO_PROCESS.terminate()
                _AUDIO_PROCESS.wait()
            return [], "media/mikasa.png"

        clear.click(
            clear_with_audio_stop,
            None,
            [chatbot, mikasa_image],
            queue=False
        )
...
def main():
    global _CONFIG, _MODEL, _TOKENIZER, _TTS_PIPELINE, _AUDIO_PROCESS, _VOICE_ENABLED
    _CONFIG = load_config()
    ...
    try:
        ...
        _VOICE_ENABLED = True
        print("✨ Voice synthesis ready! Using 'af_heart' voice")
    except Exception as e:
